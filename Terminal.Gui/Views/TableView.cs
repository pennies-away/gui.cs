...
namespace Terminal.Gui {
	...
	public class TableView : View {
		...
		private void RenderRow (int row, int rowToRender, ColumnToRender [] columnsToRender)
		{
			...
			Move (0, row);
			Driver.SetAttribute (FullRowSelect && IsSelected (0, rowToRender) ? HasFocus ? rowScheme.Focus : rowScheme.HotNormal
			...
			for (int i = 0; i < columnsToRender.Length; i++) {
				...
				var cellColor = isSelectedCell ? HasFocus ? scheme.Focus : scheme.HotNormal : Enabled ? scheme.Normal : scheme.Disabled;
				...
				if (scheme != rowScheme) {
					Driver.SetAttribute (isSelectedCell ? HasFocus ? rowScheme.Focus : rowScheme.HotNormal
						: Enabled ? rowScheme.Normal : rowScheme.Disabled);
				}
